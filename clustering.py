...
def main():
    ...
    #
    incidences = getStops(filename)
    #print(len(incidences)) #9483

    # convert categorical data to numerical data
    lines = convertToNumerical(filename2)
    writer = csv.writer(open('interest_prepped.csv', 'w'))
...
def convertToNumerical(file):
    ...
    r = csv.reader(open('interest_prepped.csv')) # Here your csv file
    lines = list(r)
    for line in lines[1:]:
        for j in range(1,19):
            curr = cols[j]
